...
class Servo:
    def __init__(self, port: str = "/dev/ttyUSB0") -> None:
        try:
            self.ser = serial.Serial(port, 38400, timeout=25)
...
def check_servo_status(servos:Servo):
    if servos is not None:
        if not servos.status:
            logging.info("舵机状态异常，状态1")
            exit(0)
        assert servos.ping(1), "舵机1状态异常"
    if servos is not None:
        xposition = 1024
        servos.revolve(1, xposition, 0, 0)
        sleep(1)
        xposition = 0
        servos.revolve(1, xposition, 0, 0)
    else:
        logging.info("舵机状态异常，状态2")
        exit(0)

if __name__ ==  "__main__":
    myservo = Servo()
    logging.info(myservo.status)